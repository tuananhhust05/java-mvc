...
public class PinController {
	...
	public CommonResponse<Object> pinPost(@RequestBody JsonNode requestBody, @RequestHeader Map<String, String> header,
		...
		PinPostRequest request = encryptService.getRequest(requestId, FunctionConstant.PIN_POST, requestBody,
				PinPostRequest.class, encrypted, deviceId);
		...
		try {
			...
			response = pinPostService.pinPost(request, user, requestId);
			return response;
		} catch (Exception e) {
			log.fatal("{} | {} | Exception {} | error = ", requestId, FunctionConstant.PIN_POST, e.getMessage(), e);
			 ...
			 */
			if (encrypted == true) {
				String encryptedData = encryptService.encrypt(requestId, FunctionConstant.PIN_POST, deviceId,
						response.getData());
				response.setData(encryptedData);
			}

			log.info("{} | {} | End | response={}", requestId, FunctionConstant.PIN_POST, gson.toJson(response));
		}
